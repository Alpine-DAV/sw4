...
% entire-USGS-h400
% ---------------------------------------------------------------------
% testname = 'berkeley-small-att-h100';
...
if (exist(file))
    ...
    img_ngrids = 3; % FIXME - read it from the file
    mat_img = cell(img_ngrids+1,10); % same read from .3Dimg files
    comp_str = ["rho", "p", "s", "qp", "qs"];
    for g=1:img_ngrids
        for comp=1:5
            file = sprintf('%s/%s-h400/image.cycle=%s.y=60000.%s.sw4img', ...
                basedir,testname,cycle_str,comp_str(comp));
            if (~exist(file))
...
%             [im,x,y,z,t,timestring,zmin]=readimage3d(file,g,0);
            [im,x,y,z,plane,t,timestring]=readimage(file, g, 1);
            mat_img{g,comp} = im;
            % Save topo too
            if (comp == 1)
...
%             grid on;
    ...
    for (f=1:5)
        fh = figure(f);
        fh.OuterPosition = [-2280 52 1288 782];
%         fh = figure(f+5);
